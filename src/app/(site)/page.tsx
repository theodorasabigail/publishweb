...
export default async function HomePage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-sea-200/70 bg-sea-950 text-cream">
        {settings.hero_image && (
          <Image
        ...
        <div className="container-page relative py-24 sm:py-32">
          <p className="text-xs uppercase tracking-[0.2em] text-sea-300">
          ...
          {settings.hero_subtitle && (
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-sea-200">
              {settings.hero_subtitle}
            </p>
          )}
          <div className="mt-9 flex flex-wrap gap-3">
            <Link
              ...
              className="btn bg-cream text-sea-950 hover:bg-white"
            >
            ...
            <Link
              href="/roasting"
              className="btn border border-sea-600 text-cream hover:bg-sea-900"
            >
              Custom roasting
            </Link>
          </div>
      ...
      {/* Value strip */}
      <section className="border-b border-sea-200/70 bg-white">
        <div className="container-page grid gap-8 py-10 sm:grid-cols-3">
          {[
            ...
            <div key={title} className="flex gap-3.5">
              <Icon className="mt-0.5 h-5 w-5 shrink-0 text-sea-800" />
              <div>
                <p className="font-medium">{title}</p>
                <p className="mt-1 text-sm text-sea-800">{body}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="container-page py-16 sm:py-20">
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sea-800">What we are roasting right now.</p>
          </div>
      ...
      {homepageCategories.length > 0 && (
        <section className="border-y border-sea-200/70 bg-white py-16 sm:py-20">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {homepageCategories.map((category) => (
                <Link
                  ...
                  className="card group flex flex-col p-6 transition-colors hover:border-sea-400"
                >
                  <p className="font-serif text-xl">{category.name}</p>
                  {category.description && (
                    <p className="mt-2 line-clamp-3 text-sm text-sea-800">
                      {category.description}
                    </p>
                  )}
                  <span className="mt-4 inline-flex items-center gap-1 text-sm text-sea-700 group-hover:gap-2">
      ...
      {/* Roasting service */}
      <section className="container-page py-16 sm:py-20">
        <div className="overflow-hidden rounded-3xl bg-sea-900 px-8 py-14 text-cream sm:px-14">
          <div className="max-w-2xl">
            ...
            <Link href="/roasting" className="btn mt-7 bg-cream text-sea-950 hover:bg-white">
  ...
  return (
    <div className="container-page py-24">
      <div className="card mx-auto max-w-2xl p-10">
        ...
        <ul className="mt-5 space-y-2 font-mono text-sm text-sea-800">
          <li>NEXT_PUBLIC_SUPABASE_URL</li>
          <li>NEXT_PUBLIC_SUPABASE_ANON_KEY</li>
          <li>SUPABASE_SERVICE_ROLE_KEY</li>
        </ul>
        <p className="mt-5 text-sm text-sea-800">
          Step-by-step instructions, including where to find each value, are in{" "}
          <code className="rounded bg-sea-100 px-1.5 py-0.5">docs/OPERATOR_SETUP.md</code>{" "}
